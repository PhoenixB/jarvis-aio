...
class JarvisPanel extends HTMLElement {
  constructor() {
    ...
    this._camMode = "stream";      // stream → still → jarvis (v7.93.1 fallback chain)
...
`;
  ...
  _renderEditableSVG(plan, floor) {
    ...
    // Placed device pins — live state; drag to move, tap to control (v7.93.1).
    var _ents = [];
    ...
    <div class="settings-grid">
      ...
      <div class="panel">
        ...
        <div class="toggle-list">
          ...
          <div class="toggle-row">
            <span class="toggle-label">Announce Important Only</span>
            <span class="toggle-desc">Auto reviews speak only for people, deliveries &amp; the like — parked cars, pets, empty scenes stay silent (still analyzed &amp; logged)</span>
            <button class="toggle-btn ${(d.config?.camera_important_only !== false) ? 'on' : 'off'}"
              data-cfg-key="camera_important_only"
              data-cfg-val="${(d.config?.camera_important_only !== false) ? 'false' : 'true'}">
              ${(d.config?.camera_important_only !== false) ? 'ON' : 'OFF'}
            </button>
          </div>
          <div class="toggle-row">
...
${this._renderExcludedEntities(d)}
      ...
      <!-- JARVIS CHARACTER + RESEARCH (v7.93.1) -->
      <div class="panel">
        <div class="head">
  ...
  _camName(entity) {
    // JARVIS-only display name (v7.93.1): camera_names map → picker name →
  ...
  _wireCameraSettings() {
    // Camera on/off toggles — choose which cameras JARVIS uses (v7.93.1).
