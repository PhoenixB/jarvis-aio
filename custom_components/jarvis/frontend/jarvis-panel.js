...
class JarvisPanel extends HTMLElement {
  constructor() {
    ...
    this._camMode = "stream";      // stream → still → jarvis (v7.87.0 fallback chain)
...
`;
  ...
  _renderEditableSVG(plan, floor) {
    ...
    // Placed device pins — live state; drag to move, tap to control (v7.87.0).
    var _ents = [];
      ...
      <div class="panel">
        ...
        <div class="home-cfg" style="margin-bottom:10px;">
          <div class="cfg-row">
            ...
            <select id="ui-lang-select" class="cfg-field" data-cfg-key="ui_language">${this._optsLabeled([['auto','Auto (Home Assistant)'],['en','English'],['zh','中文（简体）'],['cs','Čeština'],['da','Dansk'],['de','Deutsch'],['es','Español'],['fr','Français'],['it','Italiano'],['nl','Nederlands'],['nb','Norsk bokmål'],['pl','Polski'],['pt','Português'],['pt-br','Português (Brasil)'],['ro','Română'],['ru','Русский'],['sk','Slovenčina'],['fi','Suomi'],['sv','Svenska'],['tr','Türkçe'],['uk','Українська']], d.config?.ui_language || 'auto')}</select>
          </div>
        </div>
        <div class="toggle-list">
          <div class="toggle-row">
...
${this._renderExcludedEntities(d)}
      ...
      <!-- JARVIS CHARACTER + RESEARCH (v7.87.0) -->
      <div class="panel">
        <div class="head">
  ...
  _camName(entity) {
    // JARVIS-only display name (v7.87.0): camera_names map → picker name →
  ...
  _wireCameraSettings() {
    // Camera on/off toggles — choose which cameras JARVIS uses (v7.87.0).
