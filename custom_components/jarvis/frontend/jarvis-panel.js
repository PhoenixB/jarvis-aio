...
class JarvisPanel extends HTMLElement {
  constructor() {
    ...
    this._camMode = "stream";      // stream → still → jarvis (v7.88.0 fallback chain)
...
`;
  ...
  _renderEditableSVG(plan, floor) {
    ...
    // Placed device pins — live state; drag to move, tap to control (v7.88.0).
    var _ents = [];
...
${this._renderExcludedEntities(d)}
      ...
      <!-- JARVIS CHARACTER + RESEARCH (v7.88.0) -->
      <div class="panel">
        <div class="head">
  ...
  _camName(entity) {
    // JARVIS-only display name (v7.88.0): camera_names map → picker name →
  ...
  _wireCameraSettings() {
    // Camera on/off toggles — choose which cameras JARVIS uses (v7.88.0).
